...
class Plot_Manifold_Learning_Result():
    ...
    def save_images(self, images, name='result.jpg',label=1):
        images = images.reshape(self.n_img_x*self.n_img_y, self.img_h, self.img_w)
        imsave(self.DIR + "/"+name, self._merge(images, [self.n_img_y, self.n_img_x],label))

    def _merge(self, images, size,label):
        h, w = images.shape[1], images.shape[2]
        ...
        for idx, image in enumerate(images):
            ...
            if i>9 and i<80 and j>9 and j<80 :
                self.flag+=1
                imsave("test-images"+"/"+str(label)+"/"+str(self.flag)+".png",image)
            image_ = imresize(image, size=(w_, h_), interp='bicubic')
...
class Plot_Analogical_Reasoning_Result():
    ...
    def _merge(self, images, size):
        ...
        for idx, image in enumerate(images):
            ...
            print(str(i)+"hi")
            image_ = imresize(image, size=(w_, h_), interp='bicubic')
            imsave(self.DIR+"/"+"sudeept"+str(i)+".jpg",image_)
            img[j * h_:j * h_ + h_, i * w_:i * w_ + w_] = image_
