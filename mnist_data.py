...
def prepare_MNIST_data(use_norm_shift=False, use_norm_scale=True, use_data_augmentation=False):
    ...
    ind = []
    cnt1, cnt2, cnt3, cnt4, cnt0 = 0,0,0,0,0
    print(train_data.shape)
  
    for i in range(55000) :
      
        if train_labels[i][0] == 1.0 and cnt0 < 50000 :
          
            cnt0+=1
          
        elif train_labels[i][1] == 1.0 and cnt1 < 50000 :
          
            cnt1+=1
          
        elif train_labels[i][2] == 1.0 and cnt2 < 50000 :
          
            cnt2+=1
        elif train_labels[i][3] == 1.0 and cnt3 < 50000 :
          
            cnt3+=1
        elif train_labels[i][4] == 1.0 and cnt4 < 50000 :
          
            cnt4+=1
    print("Total 0 - "+str(cnt0)+ " Removing 5000")
    print("Total 1 - "+str(cnt1)+ " Removing 5500")
    print("Total 2 - "+str(cnt2)+ " Removing 5000")
    print("Total 3 - "+str(cnt3)+ " Removing 5000")
    print("Total 4 - "+str(cnt4)+ " Removing 5000")
    cnt1, cnt2, cnt3, cnt4, cnt0 = 0,0,0,0,0
    for i in range(55000) :
      
        if train_labels[i][0] == 1.0 and cnt0 < 50 :
            ind.append(i)
            cnt0+=1
          
        elif train_labels[i][1] == 1.0 and cnt1 < 55 :
            ind.append(i)
            cnt1+=1
          
        elif train_labels[i][2] == 1.0 and cnt2 < 50 :
            ind.append(i)
            cnt2+=1
        elif train_labels[i][3] == 1.0 and cnt3 < 50 :
            ind.append(i)
            cnt3+=1
        elif train_labels[i][4] == 1.0 and cnt4 < 50 :
            ind.append(i)
            cnt4+=1
    diff=0
  #print(cnt0)
    ...
    for i in ind :
        if diff%1000==0 :
            print("deleted " + str(diff))
        train_labels = numpy.delete(train_labels,i-diff,0)
        train_data = numpy.delete(train_data,i-diff,0)
        diff+=1
